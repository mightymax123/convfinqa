...
Generation is **sequential** — one conversation at a time. This is deliberate: each conversation can make up to 25 API calls through the tool-call loop — the LLM returns a tool-call request, pydantic-ai executes it locally, then sends the result back in a follow-up request. A single evaluation run can therefore generate hundreds of API calls in a short window. Concurrent generation was attempted but consistently saturated rate limits and triggered cascading exponential backoff cycles, significantly degrading throughput. Sequential execution was the reliable solution.
